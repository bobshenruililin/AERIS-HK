import type { HospitalHourState, SystemHourSnapshot } from "../types";
import type { ClusterId, HourlyClusterMetricDto } from "./types";
...
export function clusterMetricsFromSnapshot(
  ...
  return snapshot.hospitals
    .filter((h): h is HospitalHourState & { code: ClusterId } => isHospitalCluster(h.code))
    .map((hospital) => ({
      timestamp,
