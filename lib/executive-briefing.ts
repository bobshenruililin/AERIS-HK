...
export function interventionSpend(policy: PolicyState, transferredPatients: number): InterventionSpend {
  ...
  return {
    ...
    dhcHkd,
    ambulanceHkd,
    totalHkd: coolRoofHkd + sheltersHkd + dhcHkd + ambulanceHkd,
  };
}
...
export function compileExecutiveBriefing(args: {
  ...
  const spend = interventionSpend(args.policy, args.snapshot.triage?.totalTransferred ?? 0);
